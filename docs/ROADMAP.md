...
- [x] **P1.7 (M) Final experiment matrix** as suites per phase (`suite_p3_3b.yaml`, `suite_p4_32b.yaml`, `suite_p5_scale.yaml`) with the priorities of section 4; the dry-run prints the plan and its GPU-hour estimate.
- [x] **P1.8 (M) Desk check:** did arXiv:2408.12337 [4] release its traces, and under what license? Decides P4.3 (reuse vs generate). **Result (2026-09-26): no.** The paper (v1, 22 Aug 2024) has no code or data link or release statement; a GitHub repository search for its title finds nothing. Its teacher was GPT-4 (closed) and its LoRA ranks 64–1024. So P4.3 generates verified traces with the ~32B teacher (the proposal's plan), and the paper describes A8 as [4]-style distillation with this study's shared hyperparameters, not an exact reproduction.
